...
# R -  profit factor which takes the total of your winning trades divided 
#      by the absolute value of your losing trades to determine your profitability
# Max Draw Down - Maximum draw down represents how much money you lose from a recent 
#                 high before making a new high in your account
    if len(trade_data) > 0:
        #Strip Currency and , from profitAndLoss
        for i in range(len(trade_data)):
            currency = trade_data['currency'][i]
            trade_data['profitAndLoss'][i] = trade_data['profitAndLoss'][i].strip(currency)
        ...
        trading_summary_current = pd.DataFrame([[trading_date.strftime('%H:%M:%S %d-%m-%Y'),
                                                 int(trading_duration), net_profit, max_win, max_loss,num_win, num_loss, r]], 
                                               columns=['Date','Duration (min)', 'Net Profit','Max Win','Max Loss', 'Winners', 'Losers', 'R Factor'])
        trading_summary = trading_summary.append(trading_summary_current, ignore_index = True)
