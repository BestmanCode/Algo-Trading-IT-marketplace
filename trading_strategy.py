...
def MACD_Renko_RSI(MERGED_DF, RSI, l_s):
    ...
    df = copy.deepcopy(MERGED_DF)
    if l_s == '':
        if df['bar_num'].tolist()[-1] >= 2 and df['macd'].tolist()[-1] > df['macd_sig'].tolist()[-1] and df['macd_slope'].tolist()[-1] > df['macd_sig_slope'].tolist()[-1] and RSI < 30:
            signal = 'BUY'
        elif df['bar_num'].tolist()[-1] <= -2 and df['macd'].tolist()[-1] < df['macd_sig'].tolist()[-1] and df['macd_slope'].tolist()[-1] < df['macd_sig_slope'].tolist()[-1] and RSI > 70:
            signal = 'SELL'

    elif l_s == 'long':
        if df['bar_num'].tolist()[-1] <= -2 and df['macd'].tolist()[-1] < df['macd_sig'].tolist()[-1] and df['macd_slope'].tolist()[-1] < df['macd_sig_slope'].tolist()[-1] and RSI > 70:
            signal = 'Close_Sell'
        elif df['macd'].tolist()[-1] < df['macd_sig'].tolist()[-1] and df['macd_slope'].tolist()[-1] < df['macd_sig_slope'].tolist()[-1]:
            signal = 'Close'
